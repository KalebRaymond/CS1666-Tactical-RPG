...
use crate::AI::population_state::PopulationState;
use crate::unit::Unit;
...
pub fn generate_initial_population(params: &GeneticParams, units: &HashMap<(u32, u32), Unit>) -> Vec<PopulationState> {
    let mut population: Vec<PopulationState> = Vec::new();
    
    for i in 0..params.pop_num {
...
pub fn genetic_algorithm(params: &GeneticParams, units: &HashMap<(u32, u32), Unit>, current_population: &mut Vec<PopulationState>) -> Vec<PopulationState>{
    let mut initial_population = generate_initial_population(params, units);
